...
describe('TONIntegrationService', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should initialize wallet from private key if provided', async () => {
      ...
      process.env.TON_PRIVATE_KEY = '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef';
  ...
  describe('Token Information', () => {
    test('should get TON token info', async () => {
      ...
      priceOracle.getTokenPrice.mockResolvedValue(2.5);
  ...
  describe('Swap Order Operations', () => {
    beforeEach(() => {
      mockTONSDK.isWalletInitialized.mockReturnValue(true);
      mockTONSDK.getCurrentAddress.mockReturnValue('EQD4FPq-PRDieyQKkizFTRtSDyucUIqrj0v_zXJmqaDp6_0t');
    ...
    test('should handle swap order execution errors', async () => {
      mockTONSDK.sendTransaction.mockRejectedValue(new Error('Transaction failed'));
  ...
  describe('Transaction Status', () => {
    test('should get transaction status', async () => {
      ...
      const status = await tonIntegrationService.getTransactionStatus('test-hash');
    ...
    test('should return pending for non-existent transaction', async () => {
      ...
      const status = await tonIntegrationService.getTransactionStatus('non-existent-hash');
  ...
  describe('Utility Functions', () => {
    test('should validate TON address', () => {
      ...
      expect(TONSDKService.validateAddress).toHaveBeenCalledWith('valid-address');
    });
