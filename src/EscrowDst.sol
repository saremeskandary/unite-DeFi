...
contract EscrowDst is IEscrowDst {
    ...
    modifier onlySender(bytes32 escrowId) {
        require(
            destinationEscrows[escrowId].sender == msg.sender,
    ...
    function initialize(
        ...
        require(
            _destinationEscrowId != bytes32(0),
            "Invalid destination escrow ID"
        );
        ...
        emit DestinationEscrowCreated(
            _destinationEscrowId,
            _sourceEscrowId,
            _sender,
            msg.value
        );
    ...
    function completeDestinationEscrow(
        bytes32 escrowId
    ) external override onlyActive(escrowId) nonReentrant {
        ...
        require(
            block.timestamp >= escrow.startTime + escrow.timelock,
    ...
    function cancelDestinationEscrow(
        bytes32 escrowId
    ) external override onlyActive(escrowId) onlySender(escrowId) nonReentrant {
        DestinationEscrow storage escrow = destinationEscrows[escrowId];
        require(
            block.timestamp >= escrow.startTime + escrow.timelock,
            "Timelock not expired"
    ...
    function getDestinationEscrow(
        bytes32 escrowId
    ) external view override returns (DestinationEscrow memory) {
        return destinationEscrows[escrowId];
    }

    function getUserDestinationEscrows(
        address user
    ) external view override returns (bytes32[] memory) {
        return userDestinationEscrowIds[user];
    }

    function getDestinationEscrowBySource(
        bytes32 sourceEscrowId
    ) external view override returns (bytes32) {
        return sourceToDestinationMapping[sourceEscrowId];
    }

    function isDestinationExpired(
        bytes32 escrowId
    ...
    function getDestinationTimeRemaining(
        bytes32 escrowId
