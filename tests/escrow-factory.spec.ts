...
describe("1inch Escrow Factory Integration", () => {
  ...
  beforeAll(async () => {
    // Mock TronWeb for testing (avoiding actual network calls in Red phase)
    tronWeb = {
      toSun: (amount: number) => amount * 1000000,
      utils: {
        crypto: {
      ...
      contract: (abi: any[] = [], address?: string) => ({
        at: (addr: string) => ({
          isValidFactory: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          getSender: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          getRecipient: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          getTimelock: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          getAmount: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          getSourceEscrowId: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          withdraw: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          cancel: () => ({ call: () => Promise.reject(new Error("Not implemented")) }),
          getStatus: () => ({ call: () => Promise.reject(new Error("Not implemented")) })
        })
      })
    };
  });
  ...
  beforeEach(async () => {
    // Setup test accounts (mocked for testing)
    accounts = [
      "TTestAddress1111111111111111111111111",
      "TTestAddress2222222222222222222222222",
      "TTestAddress3333333333333333333333333"
    ];
  ...
  describe("Integration with existing 1inch escrow factory", () => {
    test("should connect to 1inch escrow factory contract", async () => {
      ...
      expect(() => {
        oneInchFactory = tronWeb.contract([], ONE_INCH_FACTORY_ADDRESS);
      }).not.toThrow();
  ...
  describe("Escrow contract deployment via factory", () => {
    test("should deploy escrow contract through 1inch factory", async () => {
      ...
      const deploymentParams = {
        ...
        sourceEscrowId: tronWeb.utils.crypto.keccak256("test-escrow-001"),
    ...
    test("should generate deterministic escrow addresses", async () => {
      ...
      const params = {
        sender: accounts[0],
        recipient: accounts[1],
        sourceEscrowId: tronWeb.utils.crypto.keccak256("deterministic-test"),
        salt: tronWeb.utils.crypto.keccak256("salt-123")
      };
  ...
  describe("Factory parameter passing to escrow contracts", () => {
    test("should pass correct parameters to escrow contracts", async () => {
      ...
      const escrowParams = {
        ...
        sourceEscrowId: tronWeb.utils.crypto.keccak256("param-test"),
        metadata: "cross-chain-metadata" // Simplified for testing
      };
      ...
      for (const params of invalidParams) {
        await expect(async () => {
          await escrowFactory.createEscrow(
            ...
            params.sourceEscrowId || tronWeb.utils.crypto.keccak256("test")
          ).send({
            from: accounts[0],
  ...
  describe("Cross-chain integration scenarios", () => {
    test("should handle TON to TRON escrow creation", async () => {
      ...
      const crossChainParams = {
        ...
        crossChainId: tronWeb.utils.crypto.keccak256("ton-to-tron-001")
      };
