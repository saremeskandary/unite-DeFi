...
import { TestJettonWallet } from '../build/TestJettonWallet/TestJettonWallet_TestJettonWallet';
...
describe('Jetton Integration Tests', () => {
    ...
    beforeEach(async () => {
        ...
        // Calculate jetton wallet addresses
        user1JettonWallet = await jettonMaster.getWalletAddress(user1.address);
        user2JettonWallet = await jettonMaster.getWalletAddress(user2.address);
    ...
    describe('Real Jetton Wallet Integration', () => {
        it('should create order with real jetton wallet', async () => {
            ...
            const lockJettonMsg = createLockJettonMessage(orderConfig, beginCell().storeUint(0, 32).endCell());
            ...
            expect(createResult.transactions).toHaveTransaction({
                ...
                to: tonFusion.address,
                success: false,
                exitCode: 87, // INVALID_WHITELIST (not INVALID_OWNER anymore)
            });
        });
        ...
        it('should reject expired orders with real jetton wallet', async () => {
            ...
            expect(createResult.transactions).toHaveTransaction({
                ...
                to: tonFusion.address,
                success: false,
                exitCode: 75, // ORDER_EXPIRED (not INVALID_OWNER anymore)
            });
        });
        ...
        it('should validate target chain configuration', async () => {
            ...
            const createTONToEVMResult = await tonFusion.send(
                ...
                },
                {
                    $$type: 'CreateTONToEVMOrder',
                    orderConfig: orderConfig,
                    targetChainId: 999999, // Invalid chain ID
                    customPayload: null,
                }
            );
