...
describe('Partial Fill Manager', () => {
  ...
  describe('PF-SECRET-01: Generate multiple secrets for single swap', () => {
    it('should generate multiple cryptographically secure secrets', async () => {
      const secretCount = 3;
      const secrets = await partialFillManager.generateMultipleSecrets(secretCount);
    ...
    it('should throw error for invalid secret count', async () => {
      await expect(partialFillManager.generateMultipleSecrets(-1)).rejects.toThrow();
      await expect(partialFillManager.generateMultipleSecrets(1001)).rejects.toThrow(); // Max limit
    });
  });
  ...
  describe('PF-SECRET-02: Hash management for multiple secrets', () => {
    it('should generate hash160 for each secret (Bitcoin standard)', async () => {
      ...
      const secretHashes = await partialFillManager.generateSecretHashes(secrets);
    ...
    it('should create correct mapping between secrets and hashes', async () => {
      const secrets = ['test-secret-1', 'test-secret-2'];
      const secretHashes = await partialFillManager.generateSecretHashes(secrets);
      const secretToHashMap = await partialFillManager.createSecretToHashMap(secrets);
    ...
    it('should validate secret hash format and consistency', async () => {
      ...
      const hashes = await partialFillManager.generateSecretHashes([testSecret]);
    ...
    it('should handle hash validation for known secrets', async () => {
      ...
      const isValid = await partialFillManager.validateSecretHash(testSecret, hashes[0]);
      expect(isValid).toBe(true);

      const isInvalid = await partialFillManager.validateSecretHash(testSecret, 'invalid-hash');
  ...
  describe('PF-SECRET-03: Secret management utilities', () => {
    ...
    it('should support secret rotation and replacement', async () => {
      const originalSecrets = await partialFillManager.generateMultipleSecrets(2);
      ...
      const oldHashes = await partialFillManager.generateSecretHashes(originalSecrets);
      const oldValid = await partialFillManager.validateSecretHash(originalSecrets[0], oldHashes[0]);
      expect(oldValid).toBe(false);

      // Verify new secrets are valid
      const newHashes = await partialFillManager.generateSecretHashes(newSecrets);
      const newValid = await partialFillManager.validateSecretHash(newSecrets[0], newHashes[0]);
    ...
    it('should handle secret cleanup and expiration', async () => {
      ...
      await partialFillManager.storeSecretHashesWithExpiration(secrets, hashes, 1000); // 1 second
